...
module ILoveVideoEditor
  class ListWorkflowStepTypes200Response < ApiModelBase
    attr_accessor :step_types
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ILoveVideoEditor::ListWorkflowStepTypes200Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ILoveVideoEditor::ListWorkflowStepTypes200Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'step_types')
        if (value = attributes[:'step_types']).is_a?(Array)
          self.step_types = value
        end
      else
        self.step_types = nil
      end
    ...
    def list_invalid_properties
      ...
      if @step_types.nil?
        invalid_properties.push('invalid value for "step_types", step_types cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @step_types.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] step_types Value to be assigned
    def step_types=(step_types)
      if step_types.nil?
        fail ArgumentError, 'step_types cannot be nil'
      end
