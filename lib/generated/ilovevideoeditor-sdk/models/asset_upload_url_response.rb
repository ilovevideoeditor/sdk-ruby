...
module ILoveVideoEditor
  class AssetUploadUrlResponse < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ILoveVideoEditor::AssetUploadUrlResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ILoveVideoEditor::AssetUploadUrlResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @asset_url.nil?
        invalid_properties.push('invalid value for "asset_url", asset_url cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] asset_url Value to be assigned
    def asset_url=(asset_url)
      if asset_url.nil?
        fail ArgumentError, 'asset_url cannot be nil'
      end
    ...
    def hash
      [upload_url, asset_url, path, asset_id, type].hash
    end
