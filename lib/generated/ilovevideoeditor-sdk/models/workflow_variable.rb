...
module ILoveVideoEditor
  class WorkflowVariable < ApiModelBase
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ILoveVideoEditor::WorkflowVariable` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ILoveVideoEditor::WorkflowVariable`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["string", "number", "boolean", "asset_url", "url", "text"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["string", "number", "boolean", "asset_url", "url", "text"])
    ...
    def hash
      [type, label, required, default, placeholder].hash
    end
