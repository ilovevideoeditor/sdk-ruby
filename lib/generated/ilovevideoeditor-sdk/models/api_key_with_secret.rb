...
module ILoveVideoEditor
  class ApiKeyWithSecret < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ILoveVideoEditor::ApiKeyWithSecret` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ILoveVideoEditor::ApiKeyWithSecret`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @key_data.nil?
        invalid_properties.push('invalid value for "key_data", key_data cannot be nil.')
      end
    ...
    def key_data=(key_data)
      if key_data.nil?
        fail ArgumentError, 'key_data cannot be nil'
      end
