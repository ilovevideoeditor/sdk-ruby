...
module ILoveVideoEditor
  class WorkflowDefinition < ApiModelBase
    ...
    def self.openapi_types
      {
        :'version' => :'Integer',
        :'variables' => :'Hash<String, WorkflowVariable>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ILoveVideoEditor::WorkflowDefinition` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ILoveVideoEditor::WorkflowDefinition`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @steps.length > 20
        invalid_properties.push('invalid value for "steps", number of items must be less than or equal to 20.')
      end

      if @steps.length < 1
        invalid_properties.push('invalid value for "steps", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @steps.length > 20
    ...
    def steps=(steps)
      ...
      if steps.length > 20
        fail ArgumentError, 'invalid value for "steps", number of items must be less than or equal to 20.'
      end
